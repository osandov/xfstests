...
static int compress_zlib(int in_fd, int out_fd)
{
	z_stream strm;
	Bytef in_buf[4096];
	Bytef out_buf[4096];
...
static int compress_lzo(int in_fd, int out_fd)
{
	long page_size;
	...
	for (;;) {
		...
		/* 4 for the segment header, up to 3 for padding */
		if (dst_capacity - dst_size < 7 + worst_compress) {
			char *tmp;

			do {
				dst_capacity *= 2;
			} while (dst_capacity - dst_size < 7 + worst_compress);
		...
		page_remaining = -dst_size % page_size;
		if (page_remaining < 4) {
			memset(dst + dst_size, 0, page_remaining);
			dst_size += page_remaining;
		}
		...
		le32 = htole32(dst_len);
		memcpy(dst + dst_size, &le32, 4);
		dst_size += 4 + dst_len;
	}

	le32 = htole32(dst_size);
	memcpy(dst, &le32, 4);
...
static int compress_zstd(int in_fd, int out_fd)
{
	ZSTD_CStream *zcs;
	size_t ret, in_size;
...
int main(int argc, char **argv)
{
	...
	switch (compression) {
	case ZLIB:
		ret = compress_zlib(STDIN_FILENO, STDOUT_FILENO);
		break;
	case LZO:
		ret = compress_lzo(STDIN_FILENO, STDOUT_FILENO);
		break;
	case ZSTD:
		ret = compress_zstd(STDIN_FILENO, STDOUT_FILENO);
		break;
	}
